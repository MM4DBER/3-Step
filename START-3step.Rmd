...
## This script file `START-3step.Rmd` includes ommitted code. The intention for this activity is to have trainees fill in the missing code while following along with the video. Sections with ommitted code are places in the syntax that will vary based on research context. To check your results compare with script file `FINISH-3step.Rmd`.
...
include_graphics("figures/Variables_table.png")
```
...
m_step1  <- mplusObject(
  ...
  VARIABLE = 
   "categorical = ; 
    ...
    !!! All auxiliary variables to be considered in the final model should be listed below !!!
...
### Step 2 (part 2) - Estimate the unconditional model with logits from step 1. 
...
```{r, echo=FALSE, eval=TRUE, fig.align='center', out.width="65%"}
include_graphics("figures/Distal_barplot.png")
```

### Latent class moderates effect of school `Lunch Program` (X) on `Reading` & `Math`assessments (Ys)

```{r, echo=FALSE, eval=TRUE, fig.align='center', out.width="65%"}
include_graphics("figures/Simple_slopes.png")
```
...
Retrieved from [$\color{blue}{\text{psyarxiv.com/phtxa}}$](https://psyarxiv.com/phtxa)
